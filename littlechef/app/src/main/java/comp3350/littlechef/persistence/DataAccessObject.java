...
public class DataAccessObject implements DataAccess {
    ...
    public void resetDatabase() {
        try{
            ...
            "CREATE MEMORY TABLE COOKINGTIMES(COOKINGTIMEID INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 0) NOT NULL PRIMARY KEY,COOKTIME INTEGER,RECIPEID INTEGER NOT NULL,CONSTRAINT CONSTRAINT5 FOREIGN KEY(RECIPEID) REFERENCES RECIPES(RECIPEID) ON DELETE CASCADE)\n";
            connection.createStatement().executeUpdate(cmd);
            result = checkWarning(statement, updateCount);
